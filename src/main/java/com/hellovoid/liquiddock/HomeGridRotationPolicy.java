...
final class HomeGridRotationPolicy {
    ...
    /**
     * LayoutTransformRuleGridChanged describes the target with horizontal/vertical counts while
     * its source occupancy is their transpose. A taller target therefore came from the horizontal
    ...
    /**
     * MIUI moves two fixed 4x2 special-widget slots before ordinary 2x2 blocks. Their row-major
     * indices differ after transpose, so ordinary blocks are mapped by ordinal among the remaining
     * free blocks instead of by hard-coded indices.
     */
    static int mapOtherWidgetBlockIndex(int targetColumns, int targetRows,
        ...
        // The hook is intentionally called only for ordinary blocks. If MIUI ever routes a
        // reserved special block through it, fail closed rather than stealing native ownership.
