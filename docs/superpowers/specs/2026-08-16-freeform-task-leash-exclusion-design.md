...
- HOME + freeform: preserve the existing HOME/background source semantics, while excluding every visible freeform task leash.
...
The uploaded HyperOS SystemUI APK contains `com.android.wm.shell.freeform.FreeformTaskListener`. Its task state contains the existing `RunningTaskInfo` and task `SurfaceControl` leash. The APK also exposes multiple `findTaskSurface*` symbols, so the implementation must not depend on a particular synthetic method suffix.
...
Both fields are treated as capability probes, not assumptions. They are resolved once and type-checked. If the class, field, field type, listener instance, task entry, or leash is unavailable, the capability is unavailable and capture falls back safely. No state is modified.
...
- `com.android.systemui`: only the freeform leash provider hooks.

The SystemUI hook observes two existing lifecycle points:

1. `com.android.systemui.SystemUIApplication.onCreate()` after the original method completes, to obtain an application `Context` for binding to the module broker service.
2. `com.android.wm.shell.freeform.FreeformTaskListener` constructors after the original constructor completes, to keep only a `WeakReference<Object>` to the current listener instance.
...
It exposes a private Binder provider implementing only a leash-query protocol. It does not keep a second task map. Each request reads the current HyperOS `mTasks` and `mLeash` in place.
...
Launcher continues to use `RunningTaskInfo` as the source of visible freeform task IDs. The existing semantics for `WINDOWING_MODE_FREEFORM` and task visibility remain unchanged.

For all visible freeform tasks, the resolver requests the corresponding leashes from the SystemUI provider and returns a single result:
...
Both SystemUI and Launcher use explicit `bindService()` to the module component. Connections are re-established after service disconnect with capped backoff. Reconnection is demand-driven so normal no-freeform operation does not create a permanent retry loop.
...
- per-task success/failure status;
- each successful `SurfaceControl` copied through `Parcel`.

This prevents the capture worker from performing an unbounded synchronous Binder wait.
...
## 5. SystemUI crash isolation

SystemUI safety is the highest-priority invariant.

Every SystemUI integration boundary is wrapped so no LiquidDock exception can escape into `SystemUIApplication`, `FreeformTaskListener`, WMShell, or a SystemUI Binder thread.
...
Missing classes, changed field layout, reflection errors, broker binding errors, or unsupported ROM structure mean only `capability unavailable`.

### 5.2 Passive observation only

All hooks are after-hooks. Original methods complete first. LiquidDock does not replace return values or stop original execution.

The listener reference is weak. LiquidDock does not keep a stale WMShell object alive.

### 5.3 Provider query guard

The provider's entire request handler is guarded by `catch (Throwable)`. Query failures are translated into a failure response when possible and never rethrown into WMShell.
...
## 6. Circuit breaker and error classes

Not every unavailable leash is an infrastructure failure.

### 6.1 Normal unavailability — no breaker count
...
- a request exceeded the 25 ms deadline.

These produce `UNAVAILABLE` for the current capture and may be retried on a later capture.

### 6.2 Infrastructure failure — breaker count

Examples:

- required `mTasks`/`mLeash` reflection structure is incompatible;
...
## 7. Binder death and reconnection
...
1. `SystemUIApplication` and `FreeformTaskListener` are observed again;
2. the SystemUI provider reconnects to the broker;
3. it registers the new provider Binder;
...
## 8. `SurfaceControl` ownership and cleanup
...
The existing `Floating Dock` name exclusion used by HyperOS mode 1 is not expanded into a new freeform name scheme. Freeform exclusion is exclusively by `SurfaceControl` leash.

## 10. Multiple freeform windows
...
## 11. Context acquisition and service binding

SystemUI does not use hidden `ActivityThread.currentApplication()` as the primary context source. The module passively observes the real `com.android.systemui.SystemUIApplication.onCreate()` instance and stores only its application context for broker binding.
...
- `SystemUiFreeformLeashProvider.java`: passive SystemUI listener/context observation plus read-only provider Binder.
- `FreeformLeashBrokerClient.java`: small connection/discovery helper shared conceptually by each hooked process; process-local state only.
...
## 13. Tests and verification gates
...
- late response ownership is cleaned up conceptually through a testable response-state abstraction;
...
- SystemUI hooks are passive after-hooks and do not register a `TaskOrganizer`;
...
4. HOME + freeform: existing HOME semantics remain; freeform is excluded.
...
9. Rotation with freeform present: no regression to the previously fixed black-frame/rotation behavior.
10. Workstation, All Apps, Recents, and normal Dock capture regressions remain unchanged.

SystemUI stability is a release gate: any uncaught LiquidDock exception in SystemUI, ANR attributable to the bridge, or WMShell state mutation fails the fix even if visual capture is correct.
...
3. Failure of SystemUI injection, reflection, broker binding, Binder transport, timeout, or leash lookup cannot crash SystemUI or Launcher and results in the existing WALLPAPER safety fallback.
